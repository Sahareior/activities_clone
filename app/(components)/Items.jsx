...
const Items = () => {
  ...
  return (
    <div className="space-y-4 min-h-[70vh]">
      <div>
        <div className="my-9 min-h-[60px]">
          <h2 className="mex text-[18px] text-[#000000] mb-2 capitalize min-h-[24px]">
            {activeTab?.replace("-", " ") || "3 piece"}
          </h2>
          <p className="text-gray-600 mreg text-sm min-h-[20px]">
            {activeTab === "jeans"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {finalData.slice(0, visibleItems).map((session) => (
            ...
            >
              {/* Text Content */}
              <div className="p-4 w-1/2 flex flex-col justify-center min-h-[128px]">
                <h3 className="text-[16px] font-bold text-[#000000] leading-[1.2] min-h-[20px]">
                  {session.title}
                </h3>
                <p className="text-slate-600 msemi mt-1 text-[12px] min-h-[16px]">
                  {session.price} TK
                </p>
              </div>
